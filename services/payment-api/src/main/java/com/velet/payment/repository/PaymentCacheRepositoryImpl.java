...
public class PaymentCacheRepositoryImpl implements PaymentCacheRepository {
    ...
    private static final Duration PAYMENT_TTL = Duration.ofSeconds(2 * 60);
    ...
    @Override
    public void putPending(String idempotencyKey) {
        String idxKey = PAYMENT_IDEMPOTENCY_KEY_PREFIX + idempotencyKey;
        redisTemplate.opsForValue().set(idxKey, PaymentStatus.PENDING.name(), PAYMENT_TTL);
        log.debug("payment.cache.putPending idempotencyKey={}", idempotencyKey);
    }
    ...
    public Optional<PaymentCacheEntry> getByIdempotencyKey(String idempotencyKey) {
        String idxKey = PAYMENT_IDEMPOTENCY_KEY_PREFIX + idempotencyKey;
        Object raw = redisTemplate.opsForValue().get(idxKey);
        if (raw == null) return Optional.empty();

        String value = raw.toString();
        if (PaymentStatus.PENDING.name().equals(value)) {
            return Optional.of(PaymentCacheEntry.builder()
                                                .paymentId(null)
                                                .idempotencyKey(idempotencyKey)
                                                .status(PaymentStatus.IN_PROGRESS.name())
                                                .build());
        }

        return getById(Long.parseLong(value));
    }
